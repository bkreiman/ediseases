...
class tz_ad300_widget extends WP_Widget {
	...
	function __construct() {
		...
		/* Widget settings */
		$widget_ops = array( 'classname' => 'tz_ad300_widget', 'description' => __('A widget that allows the display and configuration of of a single 300x250 Banner.', 'legatus-theme') );

		/* Widget control settings */
		$control_ops = array( 'width' => 300, 'height' => 250, 'id_base' => 'tz_ad300_widget' );

		/* Create the widget */
		parent::__construct ( 'tz_ad300_widget', __(THEME_FULL_NAME.' Custom 300x250 Ad', 'legatus-theme'), $widget_ops, $control_ops );
	}
			...
			echo '<div class="banner">
					...
					if (is_pagetemplate_active("template-contact.php")) {
						echo '<div class="banner-info">
							<a href="'.get_page_link($contactID[0]).'" class="sponsored"><span class="icon-default">&nbsp;</span>'.__("Sponsored Advert",'legatus-theme').'<span class="icon-default">&nbsp;</span></a>
						</div>' ;
					} 
				echo '</div>';
		} elseif ( $ad ) {
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e('Title:','legatus-theme') ?></label>
			<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo $instance['title']; ?>" />
		</p>
 -->
		<!-- Ad image url: Text Input -->
		<p>
			<label for="<?php echo $this->get_field_id( 'ad' ); ?>"><?php _e('Ad image url:','legatus-theme') ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'link' ); ?>"><?php _e('Ad link url:','legatus-theme') ?></label>
