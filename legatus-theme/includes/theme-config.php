...
class OrangeThemesManagment {
	...
	function __construct($themename, $themeslug, $type=false) {
...
';
	...
	function print_options_switch($switchValue, $arrayValue) {
		switch ($switchValue) {
			...
			break;
			case 'tab':
				$this->print_tab($arrayValue, self::$tab);
			break;
			case 'closetab':
				$this->print_closetab($arrayValue, self::$tab);
			break;
			case 'meta_sub_navigation':
				$this->print_meta_sub_navigation($arrayValue, self::$tab);
			break;
			case 'sub_navigation':
				$this->print_subnavigation($arrayValue, self::$tab);
			break;
			case 'sub_tab':
				$this->print_subtab($arrayValue, self::$tab);
			break;
			case 'meta_sub_tab':
				$this->print_meta_sub_tab($arrayValue, self::$tab);
			break;
			case 'closesubtab':
				$this->print_closesubtab($arrayValue, self::$tab);
			break;
	...
	function print_layer_slider_select($value) {
		...
		?>
			<span class="select">
				...
				<?php endif; ?>
				<?php if(empty($sliders)) : ?>
					<option value=""><?php _e("You didn't create a slider yet.",'legatus-theme'); ?></option>
				<?php endif; ?>
	...
	function print_meta_sub_navigation($value, $i){
		...
		if($value['subname']){
			...
			foreach($value['subname'] as $subname ){
				if(isset($subname['page_type']) && ot_page_type_check($subname['page_type'])==true || !isset($subname['page_type'])) { //meta box check
		...
		if(((isset($value['protected'][0]["id"]) && $value['protected'][0]["id"]!="") && $protected_value==$value['protected'][0]["value"]) || (!isset($value['protected'][0]["id"]))) {
			...
			echo $this->before_item.$this->before_item_title.$value['title'].$this->after_item_title.'
				...
				echo'
					...
					<div class="message-1">
						'.__("To add content to your Hompage, drag &amp; drop blocks from Available to Active area ",'legatus-theme').'
					</div>

					<h2>'.__("Active homepage blocks",'legatus-theme').'</h2>
									...
									echo '
										<li class="active-block component dropped" rel="'.$block['type'].'" id="recordsArray_'.$block['id'].'" style="opacity: 1; z-index: 0; ">
											<div class="blocks-content clearfix">
												...
												<a href="javascript:{}" id="edit_'.$block['id'].'" class="button edit">'.__("Edit",'legatus-theme').'</a>
												<a href="javascript:{}" id="delete_'.$block['id'].'"  class="button delete del">'.__("Delete",'legatus-theme').'</a>';
												if(isset($image)) {
					...
					<h2>
						'.__("Pagebuilder Import/Export",'legatus-theme').'
					</h2>
	...
	function print_export_content($value) {
		...
		<div class="input-item-full-width clearfix">
			<label><?php echo $value['title'];?></label>
				<span>
					<div class="export">
						<span id="<?php echo $value['id'];?>" class="action export-content" data-type="<?php echo $value['section'];?>" style="width:100px; float:right; z-index:500;"><?php _e("Export",'legatus-theme');?></span>
					</div>
				</span>
			</div>
		<?php 
	...
	*/
	function print_import_content($value) {
		...
		<div class="input-item-full-width clearfix">
			...
			<span>
				<div class="export">
					<span id="<?php echo $value['id'];?>_button" class="action import-content" data-type="<?php echo $value['section'];?>" style="width:100px; float:right; z-index:500;"><?php _e("Import",'legatus-theme');?></span>
					...
					<span class="select">
						<select name="<?php echo $value["id"];?>[]" class="styled" multiple="multiple">
							<?php if(isset($value['default'])) { ?>
								<option value="<?php echo $value['default'][0];?>" <?php if((is_array($input_value) && in_array($value['default'][0],$input_value)) || !is_array($input_value)) { echo "selected"; } ?>><?php echo $value['default'][1];?></options>
							<?php } ?>
							<?php 
								foreach($data as $d) {
			...
			if(isset($value['page_type']) && ot_page_type_check($value['page_type'])==true || !isset($value['page_type'])) { //meta box check
				...
				?>
						<div class="row-explenation">
							<span><?php _e("This options is disabled, to enable it you must set this option in <strong>theme management panel</strong> to custom.",'legatus-theme');?></span>
						</div>
				<?php
					}		
				}		
			...
			?>
					<li class="row-item-full-width clearfix" id="recordsArray_<?php echo convert_to_class($sidebar); ?>" alt="<?php echo $sidebar; ?>">
						<div class="blocks-content clearfix" style="text-align: left;">
							<?php _e("Sidebar name:",'legatus-theme');?> <b><?php echo $sidebar; ?></b><a href="javascript:{}" class="button edit sidebar-edit" id="edit-<?php echo convert_to_class($sidebar); ?>" rel="<?php echo $sidebar; ?>">Edit</a><a href="javascript:{}" class="button delete sidebar-delete" id="delete-<?php echo convert_to_class($sidebar); ?>">Delete</a>
						</div>
					</li>
			<?php
				}	
	...
	*/
	function print_add_text($value) {
		...
		<a href="javascript:{}" class="button-1 ot-save-management" data-saved="<?php _e("Add/Save",'legatus-theme');?>" data-loading="<?php _e("Adding...",'legatus-theme');?>"><?php _e("Add/Save",'legatus-theme');?></a>
				...
				<span class="select">
					<select name="<?php echo $value["id"];?>" class="styled">
						<?php 
							if(isset($value['default']) && $value['default']!=false) {
								foreach($value['default'] as $defaults) { ?>
									<option value="<?php echo $defaults[0];?>" <?php if($input_value==$defaults[0]) { echo "selected=\"selected\""; } ?>><?php echo $defaults[1];?></option>
						<?php 
								}
							} 
						?>
						...
						<?php if(function_exists('is_woocommerce')) { ?>
							<option value="ot_woocommerce" <?php if($input_value=="ot_woocommerce") { echo "selected=\"selected\""; } ?>><?php _e("Woocommerce",'legatus-theme');?></option>
						<?php } ?>
						<?php if(function_exists("is_bbpress")) { ?>
							<option value="ot_bbpress" <?php if($input_value=="ot_bbpress") { echo "selected=\"selected\""; } ?>><?php _e("bbPRess",'legatus-theme');?></option>
						<?php } ?>
						<?php if(function_exists("is_buddypress")) { ?>
							<option value="ot_buddypress" <?php if($input_value=="ot_buddypress") { echo "selected=\"selected\""; } ?>><?php _e("BudyPress",'legatus-theme');?></option>
						<?php } ?>
	...
	*/
	...
	?>
		<div class="row">
			<input type="hidden" name="action" value="save" /><a href="javascript:{}" class="button-1 ot-save-management" data-saved="<?php echo $value['title'];?>" data-loading="<?php _e("Saving...",'legatus-theme');?>"><?php echo $value['title'];?></a>
		</div>
	<?php
	}
